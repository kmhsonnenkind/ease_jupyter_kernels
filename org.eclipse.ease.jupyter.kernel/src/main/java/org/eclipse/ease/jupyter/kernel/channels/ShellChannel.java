...
public class ShellChannel extends AbstractRunningServerChannel {
	 ...
	 */
	public ShellChannel(final String address, final Session session,
			Kernel kernel, IOPubChannel ioPub, StdinChannel stdin) {
		super(address, session);

		// Create message handler factory.
		fMessageHandlerFactory = new AbstractMessageHandlerFactory(kernel,
				this, ioPub, stdin);
	}
	...
	protected void handleMessage(Message message) {
		...
		IMessageHandlerFactory factory = fMessageHandlerFactory
				.getHandlerFactory(messageType);
